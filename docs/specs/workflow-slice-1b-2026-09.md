...
- 文档规则与「本次约定」只解释口径，**不编译进 SQL**；执行的只有维护者映射。结果行
  只写实际执行的那条口径，其余规则列在"系统不核对两者是否一致"的说明里。时间窗口
  等规则真正进入查询，属于切片 1C。
- 用户采用的文档写法与选定的可执行口径**不做一致性校验**——系统读不懂一句手册的
  意思；两者都选完后重新展示整张确认单，由人核对。
- 规则键的归类（某句话属于统计口径还是统计周期）由模型判断。提示词写明了每个键的
  含义，是缓解不是保证；归类错了时，CLI 目前只能拒绝确认，不能就地改写一条文档规则。
