...
## 2026-08-17 · 首轮真实 eval 与调优迭代（同日晚）

- **首跑（deepseek-chat，默认温度）**：自建集 9/20。失败归因四条，全部
  是真问题：① 单 bigram 弱匹配把口径过滤器带进无关问题（"一共有多少
  用户"被剔了测试账号）→ 匹配加最低分阈值 2；② 追问判断正确但在默认
  温度下不稳定 → 加 llm.temperature 配置并在示例 config 钉 0，追问
  指引改为命令式三步协议；③ "最近N天"滚动窗口约定模型间不一致 →
  用例改自然月；④ runner 取最后一条成功 SQL 当答案 SQL，误伤答完后
  跑验证查询的良性行为 → 改为任一成功 SQL 命中即通过。
- **复跑**：首次执行 15/18（83%），自修正后 18/18（100%），追问行为
  4/4（该问的问了、对照组没乱问），口径标注 3/4。调优只动自建集。
- **外部锚点（只跑一次，零调参）**：BIRD mini-dev 30 题（seed=42，
  subset.json 进 repo；evidence 按官方协议折进问题；public 模式不加载
...
## 2026-09-09 · v0.5.1 可靠性修复 + 切片 1A/1B（企业工作流线）

- **输入**：接手一份完整交接文档，断点在"可靠性修复完成、四轮 dev100 对照
  归档"，RAG/MCP/Web/强制确认均未实施。
- **动机**：产品的真实痛点不是不会写 SQL，而是同一个业务数字由不同 RD 按不同
  方法提取、文档本身也互相矛盾。因此先立可信执行边界（1A），再让文档证据真正
  进入草案（1B）。
- **交付**：v0.5.1（可靠性修复 + 四轮证据）；切片 1A 确认门（`queryagent
  flow`）；切片 1B 文档证据（`queryagent kb`，「文档依据」带出处）。
- **关键决定**：
  - 模型返回**菜单下标**而非引用字符串。编造引用、引用未检索到/无权片段三件事
    因此不是被检出的，而是无法表达的。
  - **解析失败整份失败，规则失败逐条丢弃**。后者会渲染成"文档没说清楚"——那是
    关于业务的断言，解析 bug 不能被渲染成它。
  - 引用**只有稳定部分进定义哈希**，`doc_version` 走 `check_refs` 独立通道。
    否则改个错别字就让所有历史确认失效，用户开始盲点确认，安全性净下降。
  - 依赖极简延续：DOCX 用 stdlib `zipfile`+`ElementTree`（不引 python-docx），
    embedding 走 OpenAI 兼容协议手写 httpx（不引向量库、不引 numpy）。
  - ADR-007 **收窄而非违反** ADR-002：后者决定的是 metrics 匹配方式，仍然有效。
- **刻意没做**：语义蕴含校验（不可确定性证明，不宣称）、OCR、文档级主体白名单、
  增量同步、reranker、企业知识库适配器、统一预算、MCP/Web。
- **踩坑记录**（都是跑真实数据才暴露、单测断言看不出来的）：
  - 归一化把标题和正文粘成 `新增用户口径运营口径按…`——不仅难读，还**在边界处
    制造出任何文档里都不存在的子串**，而引用校验会痛快地确认它。
  - 出处查表 key 不匹配（引用串带 quote span、检索结果不带），确认单上出现
    **没有出处的规则**，恰恰是这一层要消灭的东西。
  - 确认单把规则名显示成 `counting_basis`。产品/运营复述不出来，而"能复述"正是
    这块屏幕的验收标准。
  - 业务空间写错时静默检索不到，看起来和"文档没说"一模一样。
- **AI 辅助边界**：本轮代码由 Claude 辅助实现，人工确认了设计取舍与验收口径。
  简历表述只引用已冻结报告的数字，不把未测量项写成已验证。
- **验证**：ruff + mypy 56 文件 + pytest 437 passed / 1 skipped；真机 DeepSeek
  验证「文档依据」、权限隔离、注入抵抗三个场景；关键词检索 Recall@3 = 9/13
  （语义未测量，缺 embedding key）。
- **2026-09-10 补测（拿到 embedding key 后）**：语义检索（BAAI/bge-m3）Recall@3
  = 12/13，关键词 9/13。**同时发现一个只在真模型上才暴露的缺陷**：余弦下限 0.35
  在假向量单测里全部通过，真机却让 8 条不相关问题中的 6 条拿回"证据"（「今天
  天气怎么样」0.429 命中运营手册）。正确证据最低 0.535、不相关最高 0.549，两个
  分布重叠，没有能分干净的阈值。下限改为可配置、bge-m3 默认 0.5（误检 1/8，
  样本内选定）。顺带补上一处文档与实现不符：README 说语义检索"配置即可开启"，
  实际配置加载器从未读取 `embedding` 段，`flow` 永远只走关键词。
  教训：**阈值类参数只在假数据上测过，等于没测**。
- **2026-09-10 发版前核对（对照 Destination 逐条）**：五个已推送的 1B 提交逐个在
  干净 worktree 里重测，全部通过（其中四个此前从未跑过 CI——一次推送只跑最新提交）。
  但 Destination 有两条**只在单测里成立**：文档分歧在 CLI 配置下被静默丢弃（多加一份
  互相矛盾的手册，原有的文档依据反而消失），`required_rules` 从未实现（"文档没说"
  无法列为缺失）。真机又抓到两处诚实性问题：结果行把 SQL 未实现的规则写成已确认口径；
  提示词只列键名，模型把归属日期归成统计周期。全部修掉后再发 v0.6.0。
  教训：**测试没走到产品路径，就等于没测**——单测传了必需键，产品传的是空元组。

## 2026-09-10 · 切片 1C 统计区间进入 SQL

- **动机**：v0.6.0 的结果行要为时间免责——问「上个月」，跑的是全量 COUNT。那句免责是
  诚实的，但它说明产品在最常见的一类取数上答的不是用户问的问题。
- **关键决定**：类型化编译而非参数绑定（ADR-008）；`period`（统计区间）与文档的
  `time_window`（统计周期约定）分开，只有用户能设；确定性解析一个封闭集合，歧义即缺口；
  绝对日期在准备草案时算出并写进哈希，第二天执行不漂移。
- **踩坑记录**：`'2026-08-01 00:00:00'` 形式的下界在 SQLite 里大于纯日期文本
  `'2026-08-01'`，每个区间首日整日漏算。编译器自己的边界测试每行都带时分秒，没抓到；是
  CLI 夹具存了纯日期才让计数变成 0。**测试数据的形状决定了测试能看见什么。**
- **验证**：546 passed / 1 skipped；MySQL / ClickHouse 与各自原生月份函数一致；真机四个
  场景与 `strftime` 独立参照一致（方案 §8）。
- **刻意没做**：按天 / 维度分组、参数绑定、数据截止日期提示、存储时区换算。

## 2026-09-11 · 切片 1C-2 结果说清覆盖、分组与文档核对（v0.8.0）

- **动机**：v0.7.0 列出的四处"还做不到"：说不出数据到哪天（不完整月当整月报）、不能按天
  或维度分组、日期拼进 SQL、文档规则与执行互不相认（结果行只能说"系统不核对"）。
- **关键决定**：日期改为绑定参数，`?` 占位、方言知识留在连接器，`%` 由词法成对（ADR-009
  取代 ADR-008）；新鲜度只说"最新一条记录在哪天"，探测放在确认之后，所以提醒只能出现在
  结果上；分组是口径、只能来自用户，「日均」问而不猜，维度按表声明、不替人 join；文档依然
  不进 SQL，但能经由维护者在映射上声明的字眼（`enforces`）选择可执行口径，对应按字眼不按
  语义，命中两种写法就不声称。
- **踩坑记录**：旧 P10 测试用 2030 年区间测"空结果"，它其实落在夹具数据之后——新行为把它
  判成"还没有数据"才暴露出来；确认单把维度显示成键名 channel；演示词表里「扣除」同时出现在
  「不扣除退款」中，差点让净额口径的对应失效。**测试夹具与词表都是规格的一部分，写错了测试
  照样绿。**
- **验证**：682 passed / 1 skipped；三方言各用另一套原生函数对照（绑定、新鲜度、三种粒度）；
  真机：按渠道、按周两种拆法合计都等于不分组的 5812；DeepSeek 抽取下文档预选、采用即选定、
  推翻文档后点名不一致，各场景见方案 §8。
- **刻意没做**：多维交叉分组、日均等派生指标、按维度取值过滤、数据完整性判断、跨表维度、
  文档规则的语义蕴含判断、存储时区换算。
- **发版前双轴 review**：Spec 轴抓到 F9 只做了一半（未声明维度被静默丢弃）、探测失败时把
  "无数据"标成"无记录"、F13 只在 CLI 生效；Standards 轴抓到新草案 v0.7 读不回、分组词误判
  （「部分渠道」成了按渠道）。全部修掉后发 v0.8.0。**自己写的验收，自己最难看出没满足。**
